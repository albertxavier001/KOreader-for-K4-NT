...
function ReaderMenu:onShowReaderMenu()
    ...
    if Device:isTouchDevice() then
        ...
        local myItems = {}
        local reflow_item = {
            text = _("Toggle Reflow"),
            callback = function()
                if self.ui.document.configurable.text_wrap == 0 then 
                    self.ui.document.configurable.text_wrap = 1
                else
        ...
        local quality_low_item = {
            text = _("Quality Low"),
            callback = function()
                self.ui.document.configurable.text_wrap = 1
                self.ui.document.configurable.quality = 0.5
                self.ui:handleEvent(Event:new("RedrawCurrentPage"))
                self.ui:handleEvent(Event:new("RestoreZoomMode"))
                self.ui:handleEvent(Event:new("InitScrollPageStates"))
            end,
        }
        local quality_mid_item = {
            text = _("Quality Midiuem"),
            callback = function()
                self.ui.document.configurable.text_wrap = 1
                self.ui.document.configurable.quality = 1.0
                self.ui:handleEvent(Event:new("RedrawCurrentPage"))
                self.ui:handleEvent(Event:new("RestoreZoomMode"))
                self.ui:handleEvent(Event:new("InitScrollPageStates"))
            end,
        }
        local quality_high_item = {
            text = _("Quality High"),
            callback = function()
                self.ui.document.configurable.text_wrap = 1
                self.ui.document.configurable.quality = 1.5
                self.ui:handleEvent(Event:new("RedrawCurrentPage"))
                self.ui:handleEvent(Event:new("RestoreZoomMode"))
                self.ui:handleEvent(Event:new("InitScrollPageStates"))
            end,
        ...
        local large_frontsize_item = {
            text = _("Font Size Increase"),
            callback = function()
                self.ui.document.configurable.text_wrap = 1
                self.ui.document.configurable.font_size = self.ui.document.configurable.font_size + 0.2
                if self.ui.document.configurable.font_size > 2.0 then
                    self.ui.document.configurable.font_size = 2.0
                end
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local smaller_frontsize_item = {
            text = _("Font Size Decrease"),
            callback = function()
                self.ui.document.configurable.text_wrap = 1
                self.ui.document.configurable.font_size = self.ui.document.configurable.font_size - 0.2
                if self.ui.document.configurable.font_size < 0.2 then
                    self.ui.document.configurable.font_size = 0.2
                end
        ...
        local frontsize28_item = {
            text = _("  Font Size 28"),
            callback = function()
                self.ui.document.configurable.font_size = 0.4
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize32_item = {
            text = _("  Font Size 32"),
            callback = function()
                self.ui.document.configurable.font_size = 0.6
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize34_item = {
            text = _("  Font Size 34"),
            callback = function()
                self.ui.document.configurable.font_size = 0.8
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize36_item = {
            text = _("  Font Size 36"),
            callback = function()
                self.ui.document.configurable.font_size = 1.0
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize38_item = {
            text = _("  Font Size 38"),
            callback = function()
                self.ui.document.configurable.font_size = 1.2
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize42_item = {
            text = _("  Font Size 42"),
            callback = function()
                self.ui.document.configurable.font_size = 1.6
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize44_item = {
            text = _("  Font Size 44"),
            callback = function()
                self.ui.document.configurable.font_size = 1.8
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
            end,
        }
        local frontsize46_item = {
            text = _("  Font Size 46"),
            callback = function()
                self.ui.document.configurable.font_size = 2.0
                self.ui:handleEvent(Event:new("FontSizeUpdate"))
        ...
        local font_finetune_larger_item = {
            text = _("Font Finetune Increase"),
            callback = function()
                self.ui.document.configurable.font_fine_tune = 0.05
                self.ui:handleEvent(Event:new("FineTuningFontSize", 0.05))
            end,
        }
        local font_finetune_smaller_item = {
            text = _("Font Finetune Decrease"),
            callback = function()
                self.ui.document.configurable.font_fine_tune = -0.05
                self.ui:handleEvent(Event:new("FineTuningFontSize", -0.05))
            end,
        ...
        ---
        local screen_portrait_item = {
            text = _("Screen Portrait"),
            callback = function()
                self.ui.document.configurable.screen_mode = "portrait"
                self.ui:handleEvent(Event:new("SetScreenMode", "portrait"))
            end,
        }
        
        local screen_landscape_item = {
            text = _("Screen Landscape"),
            callback = function()
                self.ui.document.configurable.screen_mode = "landscape"
        ...
        table.insert(myItems, screen_portrait_item)
        table.insert(myItems, screen_landscape_item)


        table.insert(myItems, reflow_item)
        table.insert(myItems, quality_low_item)
        table.insert(myItems, quality_mid_item)
        table.insert(myItems, quality_high_item)


        table.insert(myItems, smaller_frontsize_item)
        table.insert(myItems, large_frontsize_item)

        table.insert(myItems, font_finetune_smaller_item)
        table.insert(myItems, font_finetune_larger_item)

        table.insert(myItems, frontsize24_item)
        table.insert(myItems, frontsize28_item)
        table.insert(myItems, frontsize32_item)
        table.insert(myItems, frontsize34_item)
        table.insert(myItems, frontsize36_item)
        table.insert(myItems, frontsize38_item)
        table.insert(myItems, frontsize42_item)
        table.insert(myItems, frontsize44_item)
        table.insert(myItems, frontsize46_item)
